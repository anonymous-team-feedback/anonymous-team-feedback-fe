    ...
    .then(res => {
      if (res.status === 400) {
        dispatch({
          ...
          payload: { message: "No team found with that slug" }
        });
      } else {
        dispatch({
...
export const getTeamData = id => async dispatch => {
  ...
  return axios
    ...
    .catch(err =>
      dispatch({ type: GET_TEAM_DATA_FAIL, payload: { error: err } })
    );
};
...
export const approvePending = (user, user_id, request_id) => async dispatch => {
  ...
  return axios
    .put(
      `${host}jointeam/`,
      { user: user, user_id: user_id, request_id: request_id },
      { headers: { "x-auth-token": authInfo.token } }
    )
    .then(res => {
...
export const getMembers = slug => async dispatch => {
  const authInfo = await getAuthInfo();
  ...
  return axios
    ...
    .catch(err => dispatch({ type: GET_MEMBERS_FAIL, payload: err }));
};
